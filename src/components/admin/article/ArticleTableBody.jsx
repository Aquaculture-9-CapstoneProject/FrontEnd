...
export default function ArticleTableBody({ articles }) {
  ...
  const { loadingDelete } = useAdminArticleStore();
          ...
          <tr key={article.ID}>
            ...
            <td className="border-b border-neutral-4">
              <div className="flex items-center gap-2">
                ...
                <button onClick={() => handleOpenDeleteModal(article.ID)}>
                  {loadingDelete === article.ID ? (
                    <span className="loading loading-spinner loading-xs text-primary-4"></span>
                  ) : (
                    <svg
                      width="12"
                      height="16"
